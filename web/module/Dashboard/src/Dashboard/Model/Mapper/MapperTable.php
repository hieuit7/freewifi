    ...
    public function customGetData($page= 1,$limit = 10,$wheres = array(),$orders = array(),$joins = array(),&$paging) {
        ...
        if(is_array($joins)):
            foreach ($joins as $join) {
                $selector->join(array($join['alias']=>$join['table']), $join['on'], $join['column'], $join['type']);
            }
        endif;
        ...
        $PaginatorAdapter = new DbSelect($selector,$this->tableGateway->getAdapter(),$resulSetPrototype);
